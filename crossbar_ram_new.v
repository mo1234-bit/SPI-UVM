module crossbar (
    ...
    input  wire [9:0] addr_read_conv, addr_read_add, addr_read_relu, addr_read_pool, addr_read_src_A,
    ...
    output wire unvalid,
    output wire unvalid1,
    output wire unvalid2,
    output wire unvalid3            // 8-bit input
);
    ...
    ram #(.DATA_WIDTH(16), .ADDR_WIDTH(10)) ram_conv (
        .clk(clk),
        .reset(reset),
        .data_in(conv_out),
        .addr(addr_conv),
        ...
        .addr_rd(addr_read_conv),
        .data_out(conv_out_ram)
    );

    ram #(.DATA_WIDTH(32), .ADDR_WIDTH(10)) ram_add (
        .clk(clk),
        .reset(reset),
        .data_in(add_out),
        .addr(addr_add),
        ...
        .addr_rd(addr_read_add),
        .data_out(add_out_ram)
    );

    ram #(.DATA_WIDTH(16), .ADDR_WIDTH(10)) ram_relu (
        .clk(clk),
        .reset(reset),
        .data_in(relu_out),
        .addr(addr_relu),
        ...
        .addr_rd(addr_read_relu),
        .data_out(relu_out_ram)
    );

    ram #(.DATA_WIDTH(16), .ADDR_WIDTH(10)) ram_pool (
        .clk(clk),
        .reset(reset),
        .data_in(pool_out),
        .addr(addr_pool),
        ...
        .addr_rd(addr_read_pool),
        .data_out(pool_out_ram)
    );

    ram #(.DATA_WIDTH(32), .ADDR_WIDTH(10)) ram_src_A (
        .clk(clk),
        .reset(reset),
        .data_in(src_A),
        .addr(addr_src_A),
    ...
    assign add_in = (sel_Add == 5'b00000) ? src_A :
                    (sel_Add == 5'b00001) ? {16'b0, conv_out} :
                    (sel_Add == 5'b00010) ? add_out :
                    (sel_Add == 5'b00011) ? {16'b0, pool_out} :
                    (sel_Add == 5'b00100) ? {16'b0, relu_out} :
                    (sel_Add == 5'b00101) ? src_A_out_ram :
                    (sel_Add == 5'b00110) ? {16'b0, conv_out_ram} :
                    (sel_Add == 5'b00111) ? add_out_ram :
                    (sel_Add == 5'b01000) ? {16'b0, pool_out_ram} :
                    (sel_Add == 5'b01001) ? {16'b0, relu_out_ram} :
                    32'b0; 
    ...
    assign result = (sel_Res == 5'b00000) ? src_A :
                    (sel_Res == 5'b00001) ? {16'b0, conv_out} :
                    (sel_Res == 5'b00010) ? add_out :  
                    (sel_Res == 5'b00011) ? {16'b0, relu_out} :
                    (sel_Res == 5'b00100) ? {16'b0, pool_out} :
                    (sel_Res == 5'b00101) ? src_A_out_ram :
                    (sel_Res == 5'b00110) ? {16'b0, conv_out_ram} :
                    (sel_Res == 5'b00111) ? add_out_ram :  
                    (sel_Res == 5'b01000) ? {16'b0, relu_out_ram} :
                    (sel_Res == 5'b01001) ? {16'b0, pool_out_ram} :
                    32'b0;

    // Validity checks
    assign unvalid = (sel_Conv > 5'b11011) ? 1'b1 : 1'b0;
    assign unvalid1 = (sel_Pool > 5'b01101) ? 1'b1 : 1'b0;
    assign unvalid2 = (sel_Relu > 5'b01101) ? 1'b1 : 1'b0;
    assign unvalid3 = (sel_Add > 5'b01001) ? 1'b1 : 1'b0;
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            if ((src_A != 32'h0  && src_A != 32'hz) || (src_A != prev_src_A)) begin
                indicator <= 1'b1;
                indicator1 <= 1'b1;
                indicator2 <= 1'b1;
                counter <= 2'b00; 
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            if ((relu_in != 16'h0 && relu_in != 16'hx && relu_in != 16'hz) || (relu_in != prev_RELU)) begin
                indicator4 <= 1'b1; 
            end
            else begin
                indicator4 <= 1'b0;
