...
`default_nettype none
  ...
  module cache(clk,
    ...
    parameter cache_entry = 9;                  // tag = (rs1 + 23'd1)
    input    wire    clk, rst;                 // {3freet} = tag - 23'd1
                                                  // rs1 = 3freet[4:0];

    input wire[24:0]  i_p_addr;                //full address {tag, rs2[3:2]}  (where rs2[3:2] is the word enable)
    input wire[3:0]   i_p_byte_en;              //two bits to the decoder (make it) (rs2[1:0])
    input wire[31:0]  i_p_writedata;
    input     wire     i_p_read, i_p_write;     // to tell him the cache to read or write operation
    output reg [31:0]  o_p_readdata;
    output reg         o_p_readdata_valid;    // for data fetcher to tell there is a data so ready to recieve it
    output      wire  o_p_waitrequest;        // signal tell me the cache is ready to recieve data


     output wire[7:0]     hit;
    output reg [25:0]  o_m_addr;                // recieve from the cache (you will recieve 25 bit and "0" as the 26th bit)
    output  wire[3:0]  o_m_byte_en;             // these should be 2 bits and return them to the original address and send them to the eai as 32 bit {22'd0, rs1,rs2}
    output reg [127:0] o_m_writedata;              
    output reg         o_m_read, o_m_write;    //read and write to me to send it to eai as wr_en, or rd_en
    input  wire[127:0] i_m_readdata;                     
    input      wire    i_m_readdata_valid;     //tell the cache that the data fetcher is ready to send data or recieve data from it (cache is fadya (enable cache to send or recieve))
    input       wire   i_m_waitrequest;       //busy untill the signal is zero (e3ml mn banha w msh5ol l7d ma a5lehalik b 0)
...
`endif
    ...
    simple_ram #(.width(8), .widthad(cache_entry)) ram_hot(clk,  addr[cache_entry-1:0], w_cm,  w_cm_data, addr[cache_entry-1:0], r_cm_data);
                                                         //clk   wraddr                 wr_en, wr_data    rd_addr                rd_data
    ...
    assign addr = (o_p_waitrequest) ? write_addr_buf[24:2] : i_p_addr[24:2]; // set module input addr is 23bit 
       assign byte_en = (write0|write2|write3|write4|write5|write6|write7) ? 4'b1111 :byte_en_buf;
    ...
    assign invalid_num = (!valid[0]) ? 0 : 
                         ...
                         (!valid[5]) ? 5 : 
                         (!valid[6]) ? 6 : 7;
    assign word_en = (write0|write2|write3|write4|write5|write6|write7) ? 4'b1111 : 
